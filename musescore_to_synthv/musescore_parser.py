...
def parse_time_signature(v):
    n = d = ""
    for t in v.findall("./"):
        print(txt.TAB * 4, txt.CBEIGE + t.tag + txt.CEND,
...
def parse_chord(v):
    duration = ""
    for c in v.findall("./"):
        print(txt.TAB * 4, txt.CBEIGE + c.tag + txt.CEND,
...
def parse_rest(v):
    for r in v.findall("./"):
        print(txt.TAB * 4, txt.CBEIGE + r.tag + txt.CEND,
...
def parse_root(r):
    for s in r.findall("./Score/Staff"):
        print(txt.CBLUE + s.tag + txt.CEND,
              [s.attrib, s.tail.strip(), str(s.text).strip()])
        m_count = 0
        ...
        if (set_staff_start_cb):
            set_staff_start_cb()
        for m in s.findall("./"):
            print(txt.TAB * 1, txt.CGREEN + m.tag + txt.CEND, m_count,
                  [m.attrib, m.tail.strip(), str(m.text).strip()])
            if (m.tag == "Measure"):
                m_count += 1
                for w in m.findall("./"):
                    print(txt.TAB * 2, txt.CVIOLET + w.tag + txt.CEND,
                          [w.attrib, w.tail.strip(), str(w.text).strip()])
                    if (w.tag == "voice"):
                        for v in w.findall("./"):
                            print(txt.TAB * 3, txt.CYELLOW + v.tag + txt.CEND,
