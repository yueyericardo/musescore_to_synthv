...
class XMLParser:
    ...
    def set_time_signature(self, n, d):
        self._print_verbose(self.SET_TAB + txt.CRED + "set_time_signature" + txt.CEND, n, d)
    ...
    def parse_time_signature(self, v):
        n = d = ""
        for t in v.findall("./"):
            self._print_verbose(txt.TAB * 4, txt.CBEIGE + t.tag + txt.CEND, [t.attrib, t.tail.strip(), str(t.text).strip()])
            if t.tag == "sigN":
                n = t.text
            elif t.tag == "sigD":
                d = t.text
    ...
    def set_pitch(self, pitch, duration):
        self._print_verbose(self.SET_TAB + txt.CRED + "set_pitch" + txt.CEND, pitch, duration)
    ...
    def parse_chord(self, v):
        duration = ""
        for c in v.findall("./"):
            self._print_verbose(txt.TAB * 4, txt.CBEIGE + c.tag + txt.CEND, [c.attrib, c.tail.strip(), str(c.text).strip()])
            if c.tag == "durationType":
                ...
                for l in c.findall("./"):
                    self._print_verbose(txt.TAB * 5, txt.CRED2 + l.tag + txt.CEND, [l.attrib, l.tail.strip(), str(l.text).strip()])
                    if l.tag == "text" and l.text is not None:
                        self.set_lyric(l.text)
            elif c.tag == "Note":
                for n in c.findall("./"):
                    self._print_verbose(txt.TAB * 5, txt.CRED2 + n.tag + txt.CEND, [n.attrib, n.tail.strip(), str(n.text).strip()])
                    if n.tag == "Spanner" and n.attrib['type'] == "Tie":
                        for s in n.findall("./"):
                            self._print_verbose(txt.TAB * 6, s.tag, [s.attrib, s.tail.strip(), str(s.text).strip()])
                            if s.tag == "next":
                                self.set_tie()
                    elif n.tag == "pitch":
    ...
    def parse_rest(self, v):
        for r in v.findall("./"):
            self._print_verbose(txt.TAB * 4, txt.CBEIGE + r.tag + txt.CEND, [r.attrib, r.tail.strip(), str(r.text).strip()])
    ...
    def parse_root(self, r):
        for s in r.findall("./Score/Staff"):
            self._print_verbose(txt.CBLUE + s.tag + txt.CEND, [s.attrib, s.tail.strip(), str(s.text).strip()])
            m_count = 0
            if self.set_staff_start_cb:
                self.set_staff_start_cb()
            for m in s.findall("./"):
                self._print_verbose(txt.TAB * 1, txt.CGREEN + m.tag + txt.CEND, m_count, [m.attrib, m.tail.strip(), str(m.text).strip()])
                if m.tag == "Measure":
                    m_count += 1
                    for w in m.findall("./"):
                        self._print_verbose(txt.TAB * 2, txt.CVIOLET + w.tag + txt.CEND, [w.attrib, w.tail.strip(), str(w.text).strip()])
                        if w.tag == "voice":
                            for v in w.findall("./"):
                                self._print_verbose(txt.TAB * 3, txt.CYELLOW + v.tag + txt.CEND, [v.attrib, v.tail.strip(), str(v.text).strip()])
                                if v.tag == "TimeSig":
    ...
    def parse_xml(self, xml_file, set_staff_start_func=None, set_staff_end_func=None, set_time_signature_func=None,
                  set_pitch_func=None, set_rest_func=None, set_lyric_func=None, set_tie_func=None, set_dot_func=None, set_tuplet_func=None):
